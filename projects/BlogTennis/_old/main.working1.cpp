...
int main()
{
    ...
    float4x4 transform = maths::rotate(float3{ 0, 1, 0 }, 0);
    ...
    while (is_running)
    {
        ...
        if (can_draw)
        {
            ...
            float4 p1 = transform * float4{ range(0, fw), range(0, qh), 0, 1 };
            float4 p2 = transform * float4{ range(hw, fw), range(hh, fh), 0, 1 };
            float4 p3 = transform * float4{ range(0, qw), range(hh, fh), 0, 1 };

            rasterizer.triangle(
                GL3D::Vertex{ p1.xyz, float4{ range(0, 1), range(0, 1), range(0, 1), 1 } },
                GL3D::Vertex{ p2.xyz, float4{ range(0, 1), range(0, 1), range(0, 1), 1 } },
                GL3D::Vertex{ p3.xyz, float4{ range(0, 1), range(0, 1), range(0, 1), 1 } }
            );
